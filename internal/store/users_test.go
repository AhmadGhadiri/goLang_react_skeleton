...
func TestAuthenticateUser(t *testing.T) {
	...
	authUser, err := AuthenticateUser(user.Username, user.Password)
	assert.NoError(t, err)
...
func TestAuthenticateUserInvalidUsername(t *testing.T) {
	...
	authUser, err := AuthenticateUser("invalid", user.Password)
	assert.Error(t, err)
...
func TestAuthenticateUserInvalidPassword(t *testing.T) {
	...
	authUser, err := AuthenticateUser(user.Username, "wrong password")
	assert.Error(t, err)
	assert.Nil(t, authUser)
}
